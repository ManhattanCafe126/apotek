...
class TampilanManajemenObat extends StatefulWidget {
  const TampilanManajemenObat({super.key});

  @override
  State<TampilanManajemenObat> createState() => _TampilanManajemenObatState();
}

class _TampilanManajemenObatState extends State<TampilanManajemenObat> {
  ...
  Widget _buildStatusBadge(String expDateStr) {
    final status = UtilitasKadaluarsa.getExpiryStatus(expDateStr);
  ...
  void pilihObatDariDaftar(Map<String, dynamic> data, String docId) {
    final obatTerpilih = ModelObat(
      namaObat: data['nama'] ?? '',
      batch: data['batch'] ?? '',
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TampilanTambahObat(
          initialData: obatTerpilih,
          isEdit: true,
          docId: docId,
        ),
      ),
  ...
  void ubahDetailData(int stok, Function(int) onUpdate) {
  ...
  void klikHapusObat(String docId) {
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
  ...
  Future<void> klikPerbaruiObat(String docId, int newStok) async {
    try {
      await FirebaseFirestore.instance.collection('obat').doc(docId).update({
        'jumlah_stok': newStok,
      });
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Stok berhasil diperbarui menjadi $newStok'),
    ...
    return Scaffold(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final hargaBeli = (data['harga_beli'] ?? 0) as num;
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                ElevatedButton.icon(
                                  onPressed: () => _showAddStokDialog(nama, batch),
                                ...
                                PopupMenuButton(
                                  itemBuilder: (context) => [
                                    PopupMenuItem(
                                      ...
                                      onTap: () => pilihObatDariDaftar(data, docId),
                                    ),
                                    PopupMenuItem(
                                      child: const Text(
                                        'Hapus',
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const TampilanTambahObat()),
          );
        },
        backgroundColor: Colors.green,
